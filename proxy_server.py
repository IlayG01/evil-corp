...
class ProxyServer:
    """
    provides a proxy server that can handle multiple clients, implemented with raw sockets
    the communication with client are expected to be in the following order
    ...
    NOTICE - for poc only we assume that all traffic that routed to this server contain https requests
    """
    ...
    def handle_request(self, data):
        ...
        if not is_valid_by_rDNS(ip_header.dst_address):
            print(print(colored(f'[!] GOT STACKOVERFLOW REQ', 'red')))
            return
